...
describe("validate authentication flow", () => {
	...
	mocha.test("Request new confirm email", async () => {
		const result = await axios.post(getTestAxiosUrl("v1/auth/verification/request"), {}, { headers: { authorization: accessToken } }).catch((reason) => {
	...
	mocha
		.test("Refresh JWT tokens", async () => {
			const failResult = await axios.get(getTestAxiosUrl("v1/auth/refresh"), { headers: { authorization: accessToken } }).catch((reason) => {
				return reason.response;
			});
			...
			const successResult = await axios.get(getTestAxiosUrl("v1/auth/refresh"), { headers: { authorization: refreshToken } }).catch((reason) => {
				return reason.response;
			});
			...
			const failResult2 = await axios.get(getTestAxiosUrl("v1/auth/refresh"), { headers: { authorization: accessToken } }).catch((reason) => {
				return reason.response;
			});
			assert(failResult2.status == 401, "Refreshing with a refresh token that was previously used, is illegal!");

			const successResult2 = await axios.get(getTestAxiosUrl("v1/auth/refresh"), { headers: { authorization: successResult.data.refresh } }).catch((reason) => {
				return reason.response;
			});
	...
	mocha
		.test("Request password reset", async () => {
			{
				const result = await axios.get(getTestAxiosUrl(`v1/auth/password/reset?email=${email}`)).catch((reason) => {
					return reason.response;
				});
		...
		.test("Reset password from request", async () => {
			{
				const user = await getCollection("accounts").findOne({ email });
				assert(user.passwordResetToken, "Password reset token valid");
				const result = await axios.post(getTestAxiosUrl("v1/auth/password/reset/change"), { resetKey: user.passwordResetToken, newPassword: password }).catch((reason) => {
					return reason.response;
				});
	...
	mocha
		.test("Change user password", async () => {
			{
				const result = await axios.post(getTestAxiosUrl(`v1/auth/password/change`), { oldPassword: password, uid: userId, newPassword: changedPassword }).catch((reason) => {
					return reason.response;
				});
			...
			{
				const result = await axios.post(getTestAxiosUrl("v1/auth/login"), { email, password: changedPassword }).catch((reason) => {
					return reason.response;
				});
			...
			{
				const failResult = await axios.get(getTestAxiosUrl("v1/auth/refresh"), { headers: { authorization: refreshToken } }).catch((reason) => {
					return reason.response;
				});
			...
			{
				const failResult = await axios.get(getTestAxiosUrl("v1/auth/refresh"), { headers: { authorization: refreshToken2 } }).catch((reason) => {
					return reason.response;
				});
	...
	mocha
		.test("Change user email", async () => {
			{
				const result = await axios.post(getTestAxiosUrl(`v1/auth/email/change`), { password: changedPassword, oldEmail: email, newEmail: changedEmail }).catch((reason) => {
					return reason.response;
				});
			...
			{
				const result = await axios.post(getTestAxiosUrl("v1/auth/login"), { email: changedEmail, password: changedPassword }).catch((reason) => {
					return reason.response;
				});
				assert(result.status == 200, "login");
			}
			...
			{
				const failResult = await axios.get(getTestAxiosUrl("v1/auth/refresh"), { headers: { authorization: refreshToken3 } }).catch((reason) => {
					return reason.response;
				});
				assert(failResult.status == 200, "New refresh token should work");
			}
