...
export const getFriendLevel = async (owner: string, requestor: string): Promise<FriendLevel> => {
	...
	const friendDoc = await Mongo.getCollection("friends", SimplyPluralDb).findOne({ uid: owner, frienduid: requestor });
	if (!friendDoc) {

		const pendingDoc = await Mongo.getCollection("pendingFriendRequests", SimplyPluralDb).findOne({
			$or: [
