...
export const init = (server: http.Server) => {
	...
	if (process.env.SOCKETEMIT === "true") {
		listenCollections.forEach((collection) => {
			const changeStream = Mongo.getCollection(collection, SimplyPluralDb).watch([], { fullDocument: "updateLookup" });
...
async function dispatchInner(uid: any, event: ChangeEventNative) {
	...
	if (event.operationType === OperationType.Delete) {
		...
		const document = await Mongo.getCollection(event.collection, SimplyPluralDb).findOne({ _id: Mongo.parseId(event.documentId) });
