				...
				if (status == 200) {
					if (Object.keys(memberDataToSync).length > 0) {
						const patchRequest: PkRequest = {
							...
							token,
							response: null,
							data: memberDataToSync,
							type: PkRequestType.Patch,
							id: "",
						}
	...
	if (pkMembersResult) {
		if (pkMembersResult.status === 200) {
			...
			for (let i = 0; i < foundMembers.length; ++i) {
				...
				const spMemberResult = await getCollection("members").findOne({ uid: userId, pkId: member.id }, { projection: { _id: 1 } })
