...
export const syncMemberToPk = async (options: syncOptions, spMemberId: string, token: string, userId: string): Promise<{ success: boolean, msg: string }> => {
	const spMemberResult = await getCollection("members", SimplyPluralDb).findOne({ uid: userId, _id: parseId(spMemberId) })
				...
				if (pkMemberResult.status == 200) {
					...
					if (postResult) {
						if (postResult.status === 200) {
							await getCollection("members", SimplyPluralDb).updateOne({ uid: userId, _id: parseId(spMemberId) }, { $set: { pkId: postResult.data.id } })
							return { success: true, msg: `${name} added to PluralKit` }
						}
						else {
...
export const syncMemberFromPk = async (options: syncOptions, pkMemberId: string, token: string, userId: string, memberData: any | undefined, batch: BulkWriteOperation<any>[] | undefined, privateByDefault: boolean): Promise<{ success: boolean, msg: string }> => {
	...
	const spMemberResult = await getCollection("members", SimplyPluralDb).findOne({ uid: userId, pkId: pkMemberId })
	...
	if (spMemberResult) {
		if (memberDataToSync && Object.keys(memberDataToSync).length > 0) {
			{
				if (batch) {
					...
					await getCollection("members", SimplyPluralDb).updateOne({ uid: userId, pkId: pkMemberId }, { $set: memberDataToSync }, {})
				}
			}
		...
		if (batch) {
			...
			await getCollection("members", SimplyPluralDb).insertOne(memberDataToSync)
		}
...
export const syncAllSpMembersToPk = async (options: syncOptions, _allSyncOptions: syncAllOptions, token: string, userId: string): Promise<{ success: boolean, msg: string }> => {
	const spMembersResult = await getCollection("members", SimplyPluralDb).find({ uid: userId }).toArray()
	...
	if (pkMembersResult) {
		if (pkMembersResult.status === 200) {
			...
			for (let i = 0; i < foundMembers.length; ++i) {
				...
				const spMemberResult = await getCollection("members", SimplyPluralDb).findOne({ uid: userId, pkId: parseId(member.id) })
			...
			if (bulkWrites && bulkWrites.length > 0) {
				getCollection("members", SimplyPluralDb).bulkWrite(bulkWrites);
			}
