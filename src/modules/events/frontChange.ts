...
export const frontChange = async (uid: string, removed: boolean, memberId: string) => {
	...
	const sharedCollection = getCollection("sharedFront", SimplyPluralDb);
	const privateCollection = getCollection("privateFront", SimplyPluralDb);
	const frontersCollection = getCollection("frontHistory", SimplyPluralDb);
	...
	const members = getCollection("members", SimplyPluralDb);
	const frontStatuses = getCollection("frontStatuses", SimplyPluralDb);
	...
	const friendCollection = getCollection("friends", SimplyPluralDb);
	...
	foundFriends.forEach(async (doc) => {
		...
		if (getFrontNotif) {
			...
			if (friendSettings && selfFriendSettings && friendSettings["getTheirFrontNotif"]) {
				notifyUser(doc["frienduid"], userDoc["username"], message, SimplyPluralDb);
			}
		}
	});
};
