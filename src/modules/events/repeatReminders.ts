...
export const repeatRemindersEvent = async (uid: string) => {
	const repeatReminders = getCollection("repeatedReminders", SimplyPluralDb);
	const foundReminders = await repeatReminders.find({ uid: uid }).toArray();

	const privateUserData = await getCollection("private", SimplyPluralDb).findOne({ uid: uid });
...
export const repeatRemindersDueEvent = async (uid: string, event: any) => {
	const privateUserData = await getCollection("private", SimplyPluralDb).findOne({ uid: uid });
	if (privateUserData) {
		notifyUser(uid, "Reminder", event.message, SimplyPluralDb);
		const repeatReminders = getCollection("repeatedReminders", SimplyPluralDb);
