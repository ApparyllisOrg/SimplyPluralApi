...
const decrementGenerationsLeft = async (uid: string) => {
	const user: any | null = await getCollection("users", SimplyPluralDb).findOne({ uid, _id: uid })
	const patron: boolean = user?.patron ?? false;

	const privateDoc = await getCollection("private", SimplyPluralDb).findOne({ uid, _id: uid });
	if (privateDoc.generationsLeft) {
		await getCollection("private", SimplyPluralDb).updateOne({ uid, _id: uid }, { $inc: { generationsLeft: -1 } });
	}
	else {
		await getCollection("private", SimplyPluralDb).updateOne({ uid, _id: uid }, { $set: { generationsLeft: patron ? 10 : 3 } });
	}
}

const canGenerateReport = async (res: Response): Promise<boolean> => {
	const privateDoc = await getCollection("private", SimplyPluralDb).findOne({ uid: res.locals.uid, _id: res.locals.uid });
	if (privateDoc) {
...
const performReportGeneration = async (req: Request, res: Response) => {
	...
	getCollection("reports", SimplyPluralDb).insertOne({ uid: res.locals.uid, url: reportUrl, createdAt: moment.now(), usedSettings: req.body })
...
export const get = async (req: Request, res: Response) => {
	...
	let document = await getCollection("users", SimplyPluralDb).findOne({ uid: req.params.id })
	...
	if (!document && ownDocument) {
		...
		document = await getCollection("users", SimplyPluralDb).findOne({ uid: res.locals.uid })
	}
...
export const update = async (req: Request, res: Response) => {
	...
	await getCollection("users", SimplyPluralDb).updateOne({
...
export const SetUsername = async (req: Request, res: Response) => {
	performUsernameUpdate(req, res, SimplyPluralDb)
};
...
export const deleteAccount = async (req: Request, res: Response) => {
	...
	const userDoc = await getCollection("users", SimplyPluralDb).findOne({ uid: res.locals.uid, _id: res.locals.uid })
	const username = userDoc?.username ?? "";

	const collections = await db(SimplyPluralDb)!.listCollections().toArray();
	...
	collections.forEach(async (collection) => {
		...
		await getCollection(actualName, SimplyPluralDb).deleteMany({ uid: res.locals.uid });
	});

	await getCollection("friends", SimplyPluralDb).deleteMany({ frienduid: res.locals.uid });
	await getCollection("pendingFriendRequests", SimplyPluralDb)
...
export const exportUserData = async (_req: Request, res: Response) => {
	...
	collections.forEach(async (collection) => {
		...
		const collectionData = await getCollection(actualName, SimplyPluralDb).find({ uid: res.locals.uid }).toArray();
...
export const initializeCustomFields = async (uid: string) => {
	const userDoc = await getCollection("users", SimplyPluralDb).findOne({ uid: uid });
	if (userDoc["fields"]) {
		// Already have fields, don't setup!
		return;
	}

	const memberWithFields = await getCollection("members", SimplyPluralDb).findOne({ uid: uid, info: { $exists: true } });
	if (memberWithFields) {
		update122(uid);
	}
	else {
