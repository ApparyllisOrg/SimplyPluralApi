...
export const getGroups = async (req: Request, res: Response) => {
	fetchCollection(req, res, "groups", SimplyPluralDb, {});
}

export const get = async (req: Request, res: Response) => {
	fetchSimpleDocument(req, res, "groups", SimplyPluralDb);
}

export const add = async (req: Request, res: Response) => {
	addSimpleDocument(req, res, "groups", SimplyPluralDb);
}

export const update = async (req: Request, res: Response) => {
	const group = await getCollection("groups", SimplyPluralDb).findOne({ uid: res.locals.uid, _id: parseId(req.params.id) })
	...
	updateSimpleDocument(req, res, "groups", SimplyPluralDb)
}
...
const delGroupRecursive = async (groupId: string, uid: string) => {
	const groups = await getCollection("groups", SimplyPluralDb).find({ uid, parent: groupId }).toArray()
	for (let i = 0; i < groups.length; i++) {
		await delGroupRecursive(groups[i]._id, uid)
	}
	await getCollection("groups", SimplyPluralDb).deleteOne({ uid, _id: parseId(groupId) })
	dispatchDelete({
...
const privateGroupRecursive = async (groupId: string, uid: string, priv: boolean, preventTrusted: boolean) => {
	...
	await getCollection("groups", SimplyPluralDb).updateOne({ uid, _id: groupId }, { $set: { "private": priv, preventTrusted } })
}
