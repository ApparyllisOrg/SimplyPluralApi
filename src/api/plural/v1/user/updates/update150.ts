...
export const update150 = async (uid: string) => {
	...
	const fhCollection = getCollection("frontHistory", SimplyPluralDb)
	const memberCollection = getCollection("members", SimplyPluralDb)
	...
	if (commentsToInsert.length > 0)
		await getCollection("comments",SimplyPluralDb).insertMany(commentsToInsert)

	const fronters = await getCollection("fronters", SimplyPluralDb).find({ uid }).toArray()
	...
	const polls = await getCollection("polls", SimplyPluralDb).find({ uid }).toArray();
	for (let i = 0; i < polls.length; ++i) {
		...
		await getCollection("polls", SimplyPluralDb).updateOne({ _id: parseId(poll._id), uid }, { $set: { votes } });
	}

	// Update note version
	// Colors were previously an index matching a specific color, now they are color strings
	const notes = await getCollection("notes", SimplyPluralDb).find({ uid }).toArray();
	for (let i = 0; i < notes.length; ++i) {
		...
		if (typeof color !== "string") {
			...
			await getCollection("notes", SimplyPluralDb).updateOne({ _id: parseId(note._id), uid }, { $set: { color: colorString } });
		}
	}
	...
	for (let i = 0; i < members.length; ++i) {
		...
		if (spId.length == 5 && (!pkId || pkId.length != 5)) {
			await getCollection("members",SimplyPluralDb).updateOne({ _id: spId, uid }, { $set: { pkId: spId } });
		}
	}
}