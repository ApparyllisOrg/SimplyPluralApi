...
export const generateUserReport = async (query: { [key: string]: any }, uid: string) => {
	...
	const user = await getCollection("users", SimplyPluralDb).findOne({ uid });
	...
	const members = await getCollection("members", SimplyPluralDb).find({ uid: uid }).toArray();
	members.sort((a, b) => {
	...
	const customFronts = await getCollection("frontStatuses", SimplyPluralDb).find({ uid: uid }).toArray();
	...
	if (query.frontHistory) {
		...
		const history = await getCollection("frontHistory",SimplyPluralDb).find(searchQuery).sort({ "startTime": -1 }).toArray();

		const liveFronters = await getCollection("frontHistory", SimplyPluralDb).find({ "live": true, uid }).sort({ "startTime": -1 }).toArray();
