...
export const getFrontHistoryInRange = async (req: Request, res: Response) => {
	...
	const documents: documentObject[] = await getCollection("frontHistory", SimplyPluralDb).find(query).toArray()
	sendDocuments(req, res, "frontHistory", documents);
}

export const getFrontHistory = async (req: Request, res: Response) => {
	fetchCollection(req, res, "frontHistory", SimplyPluralDb, {});
}

export const getFrontHistoryForMember = async (req: Request, res: Response) => {
	fetchCollection(req, res, "frontHistory", SimplyPluralDb, { member: req.params.id });
}

export const getFronters = async (req: Request, res: Response) => {
	const documents: documentObject[] = await getCollection("frontHistory", SimplyPluralDb).find({ uid: res.locals.uid, live: true }).toArray()
	sendDocuments(req, res, "frontHistory", documents);
}

export const get = async (req: Request, res: Response) => {
	fetchSimpleDocument(req, res, "frontHistory", SimplyPluralDb);
}

export const add = async (req: Request, res: Response) => {
	const potentiallyFrontingDoc = await getCollection("frontHistory", SimplyPluralDb).findOne({ uid: res.locals.uid, member: req.body.member, live: true })
	if (potentiallyFrontingDoc) {
		...
		await addSimpleDocument(req, res, "frontHistory", SimplyPluralDb);
		frontChange(res.locals.uid, false, req.body.member)
	}
}

export const update = async (req: Request, res: Response) => {
	const frontingDoc = await getCollection("frontHistory", SimplyPluralDb).findOne({ _id: parseId(req.params.id) })
	if (frontingDoc) {
		...
		if (req.body.member != null && req.body.member != undefined && frontingDoc.live === true) {
			...
			const alreadyFrontingDoc = await getCollection("frontHistory", SimplyPluralDb).findOne({ member: req.body.member, live: true })
		...
		await updateSimpleDocument(req, res, "frontHistory", SimplyPluralDb)

		const isCustom = await isCustomFront(res.locals.uid, req.body.member ?? frontingDoc.member);
		await getCollection("frontHistory", SimplyPluralDb).updateOne({ _id: parseId(req.params.id) }, { $set: { custom: isCustom } })
...
export const del = async (req: Request, res: Response) => {
	...
	await getCollection("comments", SimplyPluralDb).deleteMany({ documentId: req.params.id, uid: res.locals.uid, collection: "frontHistory" });
	...
	deleteSimpleDocument(req, res, "frontHistory", SimplyPluralDb);
}
