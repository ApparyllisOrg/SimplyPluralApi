...
export const getMembers = async (req: Request, res: Response) => {
	...
	const query = getCollection("members", SimplyPluralDb).find({ uid: req.params.system })
	const documents = await query.toArray();

	if (req.params.system != res.locals.uid) {
		const ownerUser = await getCollection("users", SimplyPluralDb).findOne({ uid: req.params.system });
...
export const get = async (req: Request, res: Response) => {
	fetchSimpleDocument(req, res, "members", SimplyPluralDb);
}

export const add = async (req: Request, res: Response) => {
	addSimpleDocument(req, res, "members", SimplyPluralDb);
}

export const update = async (req: Request, res: Response) => {
	updateSimpleDocument(req, res, "members", SimplyPluralDb)

	// If this member is fronting, we need to notify and update current fronters
	const fhLive = await getCollection("frontHistory", SimplyPluralDb).findOne({ uid: res.locals.uid, member: req.params.id, live: true })
	if (fhLive) {
...
export const del = async (req: Request, res: Response) => {
	...
	await getCollection("frontHistory", SimplyPluralDb).updateOne({ uid: res.locals.uid, member: req.params.id, live: true }, { $set: { live: false, endTime: moment.now() } });
	...
	getCollection("groups", SimplyPluralDb).updateMany({ uid: res.locals.uid }, { $pull: { members: req.params.id } });

	// Delete notes that belong to this member
	getCollection("notes", SimplyPluralDb).deleteMany({ uid: res.locals.uid, member: req.params.id });

	deleteSimpleDocument(req, res, "members", SimplyPluralDb);
}
