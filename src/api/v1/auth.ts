...
export const loginGoogle = async (req: Request, res: Response) => {
	const version = req.body.version !== null && req.body.version !== undefined ? req.body.version : null

	const result = await loginWithGoogle(req.body.credential, version)
...
export const loginApple = async (req: Request, res: Response) => {
	...
	const result = await loginWithApple(req.body.credential, version)
	if (result.success === true) {
		const isSuspended = await isUserSuspended(result.uid)
		if (isSuspended) {
			res.status(401).send("Your account is suspended")
