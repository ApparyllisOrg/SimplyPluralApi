...
import { getPasswordRegex, newPasswordRegexError, revokeAllUserAccess } from "./auth.core"
...
export const changePassword_Execution = async (uid: string, oldPassword: string, newPassword: string): Promise<{ success: boolean; msg: string; uid: string }> => {
	...
	if (user) {
		...
		if (!getPasswordRegex().test(newPassword)) {
			return { success: false, msg: newPasswordRegexError, uid: "" }
		}
		...
		if (bGeneratedHash.length !== knownHash.length) {
			return { success: false, msg: "Current password is incorrect", uid: "" }
		}
