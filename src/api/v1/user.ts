...
import { db, getCollection, getCollectionTyped, parseId } from "../../modules/mongo"
...
import { User } from "../types/documents/user.types"
...
export const getMe = async (req: Request, res: Response) => {
	const document = await getCollection<User>("users").findOne({ uid: res.locals.uid })
	if (document) {
...
export const deleteAccount = async (req: Request, res: Response) => {
	...
	collections.forEach(async (collection) => {
		...
		await getCollectionTyped<SimplyDocument>(actualName).deleteMany({ uid: res.locals.uid })
	})
