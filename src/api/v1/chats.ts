...
interface CategoryPrivateCollection {
	_id: ObjectId
	uid: string
	categories: string[]
}
...
export const deleteChannel = async (req: Request, res: Response) => {
	...
	const categoriesCollection: Collection<CategoryCollection> = getCollection("channelCategories")

	await categoriesCollection.updateOne({ uid: res.locals.uid }, { $pull: { channels: req.params.id.toString() } })
...
export const deleteChannelCategory = async (req: Request, res: Response) => {
	...
	const privateCollection: Collection<CategoryPrivateCollection> = getCollection("private")

	await privateCollection.updateOne({ uid: res.locals.uid, _id: res.locals.uid }, { $pull: { categories: req.params.id } })
