...
export const filterFieldsForPrivacy = async (req: Request, res: Response, uid: string, members: any[]): Promise<void> => {
	...
	if (hasMigrated) {
		...
		const userFields: CustomFieldType[] = await getCollection("customFields")
			.find({ uid, buckets: { $in: buckets } })
			.toArray()
...
export const getMembers = async (req: Request, res: Response) => {
	...
	if (req.params.system != res.locals.uid) {
		...
		if (migrated) {
			...
			// Fetch assigned buckets, fetch fields with those buckets assigned, then aggregate the members fields to exclude custom fields that are disallowed
			if (friendDoc) {
				const assignedBuckets = friendDoc.buckets ?? []

				const allowedFields = await getCollection("customFields")
					.find({ uid: req.params.system, buckets: { $in: assignedBuckets } })
					.toArray()

				const allowedFieldsList: string[] = []

				for (let i = 0; i < allowedFields.length; ++i) {
					allowedFieldsList.push(allowedFields[i]._id.toString())
				}

				const members = await getCollection("members")
					.aggregate([
						{ $match: { uid: req.params.system, buckets: { $in: assignedBuckets } } },
						{
							$set: {
								info: {
									$arrayToObject: {
										$filter: {
											...
											as: "item",
											cond: { $in: ["$$item.k", allowedFieldsList] },
										},
									},
								},
							},
				...
				// @ts-ignore
				members.forEach((member) => payloadArray.push(transformResultForClientRead(member, res.locals.uid)))
	...
	} // Send to ourselves, bypass all checks
	else {
...
export const updateInfo = async (req: Request, res: Response) => {
	...
	infoFieldsKeys = infoFieldsKeys.filter(
		(fieldKey) =>
			userFields.findIndex((userField) => {
