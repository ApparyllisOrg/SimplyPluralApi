...
export const stripeCallback = async (req: Request, res: Response) => {
	...
	case "customer.subscription.updated": {
		if (event.data.object as Stripe.Subscription) {
			...
			if (isSubscriptionCancelled(eventObject)) {
				getCollection("subscribers").updateOne({ customerId }, { $set: { cancelled: true } })
			} else {
				getCollection("subscribers").updateOne({ customerId }, { $set: { cancelled: false } })
			}

			if (eventObject.items.data.length == 1) {
				const item = eventObject.items.data[0]
				getCollection("subscribers").updateOne({ customerId }, { $set: { priceId: item.price.id } })
			}
		}
		break
	}
