...
import { fetchCollection, transformResultForClientRead } from "../../../util"
...
export const getInvoices = async (req: Request, res: Response) => {
	...
	const customer = await getCustomerIdFromUser(res.locals.uid, false)
	...
	invoices?.data.forEach((invoice) => {
		...
		if (!invoice.id) {
			Sentry.setExtra("customer", invoice.customer as string)
			Sentry.captureMessage(`Failed to get id from invoice`)
			return
		}

		if (!invoice.parent) {
			Sentry.setExtra("invoice", invoice.id)
			Sentry.captureMessage(`Failed to get parent from invoice`)
			return
		}

		if (!invoice.parent.subscription_details) {
			Sentry.setExtra("invoice", invoice.id)
			Sentry.captureMessage(`Failed to get parent sub details from invoice`)
			return
		}
