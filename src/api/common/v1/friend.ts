...
export const getFriend = async (req: Request, res: Response) => {
	const document = await getCollection("friends", SimplyPluralDb).findOne({ uid: req.params.system, frienduid: req.params.id });
	sendDocument(req, res, "friends", document);
}

export const getFriends = async (req: Request, res: Response) => {
	const friends = await getCollection("friends", SimplyPluralDb).find({ uid: res.locals.uid }).toArray();
	const friendValues: any[] = []

	for (let i = 0; i < friends.length; ++i) {
		const friend = await getCollection("users", SimplyPluralDb).findOne({ uid: friends[i].frienduid });
		if (friend) {
...
export const getIngoingFriendRequests = async (req: Request, res: Response) => {
	const documents = await getCollection("pendingFriendRequests", SimplyPluralDb).find({ receiver: res.locals.uid }).toArray();
	const friendValues: any[] = []

	for (let i = 0; i < documents.length; ++i) {
		friendValues.push(await getCollection("users", SimplyPluralDb).findOne({ uid: documents[i].sender }))
	}
...
export const getOutgoingFriendRequests = async (req: Request, res: Response) => {
	const documents = await getCollection("pendingFriendRequests", SimplyPluralDb).find({ sender: res.locals.uid }).toArray();
	const friendValues: any[] = []

	for (let i = 0; i < documents.length; ++i) {
		friendValues.push(await getCollection("users",  SimplyPluralDb).findOne({ uid: documents[i].receiver }))
	}
...
export const updateFriend = async (req: Request, res: Response) => {
	...
	const result = await getCollection("friends", SimplyPluralDb).updateOne({
...
export const getFriendFrontValues = async (req: Request, res: Response) => {
	const friends = getCollection("friends", SimplyPluralDb);

	const sharedFront = getCollection("sharedFront", SimplyPluralDb);
	const privateFront = getCollection("privateFront", SimplyPluralDb);
...
export const getFriendFront = async (req: Request, res: Response) => {
	const friendFronts = await getCollection("frontHistory", SimplyPluralDb).find({ uid: req.params.id, live: true }).toArray();
	...
	for (let i = 0; i < friendFronts.length; ++i) {
		...
		const memberDoc = await getCollection("members", SimplyPluralDb).findOne({ _id: parseId(member) });
		...
		const customFrontDoc = await getCollection("frontStatuses", SimplyPluralDb).findOne({ _id: parseId(member) });
		if (customFrontDoc) {
