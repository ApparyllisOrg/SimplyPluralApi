...
export type DiffReturn = Promise<{ processed: false; result: undefined } | { processed: true; result: DiffResult } | { processed: true; result: DiffResult[] } | { processed: true; result: undefined; ignore: true }>
...
export const logAudit = async (uid: string, _id: string | ObjectId, collection: string, t: number, lhs: any, rhs: any, processor: DiffProcessor | undefined) => {
	...
	if (typeof name !== "string" || name.length === 0) {
		// Don't log audits of nameless things, that just confuses people
		return
	}
