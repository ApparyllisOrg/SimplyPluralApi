...
import { SimplyPluralDb } from "./types";
...
export const notifyUser = async (uid: string, title: string, message: string, db: string) => {
	socketNotify(uid, title, message);

	const privateCollection = Mongo.getCollection("private", db);
				...
				messaging()
					...
					.catch((error) => {
						if (error.code === "messaging/registration-token-not-registered") {
							...
							notifyUser(uid, title, message, db)
...
export const fetchSimpleDocument = async (req: Request, res: Response, collection: string, db: string) => {
	const document = await Mongo.getCollection(collection, db).findOne({ _id: parseId(req.params.id), uid: req.params.system ?? res.locals.uid });
	sendDocument(req, res, collection, document);
}

export const deleteSimpleDocument = async (req: Request, res: Response, collection: string, db: string) => {
	const result = await Mongo.getCollection(collection, db).deleteOne({
...
export const fetchCollection = async (req: Request, res: Response, collection: string, db: string, findQuery: { [key: string]: any }, forEach?: forEachDocument) => {
	findQuery.uid = req.params.system ?? res.locals.uid;
	const query = Mongo.getCollection(collection, db).find(findQuery)
...
export const addSimpleDocument = async (req: Request, res: Response, collection: string, db: string) => {
	...
	const result = await Mongo.getCollection(collection, db).insertOne(dataObj).catch(() => {
		return {
			insertedCount: 0,
			ops: [],
...
export const updateSimpleDocument = async (req: Request, res: Response, collection: string, db: string) => {
	const dataObj: documentObject = req.body;
	dataObj.uid = res.locals.uid;
	dataObj.lastOperationTime = res.locals.operationTime;
	await Mongo.getCollection(collection, db).updateOne({
		_id: parseId(req.params.id), uid: res.locals.uid, $or: [
			{ lastOperationTime: null },
			{ lastOperationTime: { $lte: res.locals.operationTime } }
		]
...
export const isMember = async (uid: string, id: string) => {
	const member = await Mongo.getCollection("members", SimplyPluralDb).findOne({ uid, _id: parseId(id) })
	return !!member
}

export const isCustomFront = async (uid: string, id: string) => {
	const cf = await Mongo.getCollection("frontStatuses", SimplyPluralDb).findOne({ uid, _id: parseId(id) })
	return !!cf
}
