...
// Send query that has a query that preflighted the permissions by use of query params
export const sendQueryPermissionsPreFlighted = async (req: Request, res: Response, collection: string, query: internal.Readable & AsyncIterable<any>, forEach?: forEachDocument) => {
	const parseDocument = async (chunk: any) => {
		if (forEach) {
			const forEachResult = await forEach(chunk)
			if (forEachResult !== true) {
...
export const fetchCollectionPermissionsPreflighted = async (req: Request, res: Response, collection: string, findQuery: { [key: string]: any }, forEach?: forEachDocument) => {
	findQuery.uid = req.params.system ?? res.locals.uid
	...
	sendQueryPermissionsPreFlighted(req, res, collection, query.stream(), forEach)
}
...
export const getAPIUrl = (extension: string) => {
	return `${config().server.baseUrl}/${extension}`
}
...
export const convertListToIds = async (uid: string, collection: string, listOfIds: string[]): Promise<any[]> => {
	...
	const foundDocuments = await getCollection(collection)
...
export const fetchBucketsForFriend = async (friend: string, owner: string) => {
	const friendDoc = await getCollection("friends").findOne({ uid: owner, frienduid: friend })

	if (friendDoc) {
		return friendDoc.buckets ?? []
	}
