...
import { PrivacyBucket } from "../api/types/documents/privacyBucket.types"

export function transformResultForClientRead(value: SimplyDocument, requestorUid: string) {
...
export const getDocumentAccess = async (requestor: string, document: SimplyDocument, collection: string): Promise<{ access: boolean; statusCode: number; message: string }> => {
...
export const sendDocument = async (req: Request, res: Response, collection: string, document: SimplyDocument | null) => {
	if (!document) {
		res.status(404).send()
		return
	}
...
export const fetchSimpleDocument = async <T extends SimplyDocument>(req: Request, res: Response, collection: string) => {
	const document = await Mongo.getCollectionTyped<T>(collection).findOne({ _id: parseId(req.params.id), uid: req.params.system ?? res.locals.uid })
...
export const addSimpleDocument = async (req: Request, res: Response, collection: string, insertFields?: (data: any) => Promise<void>) => {
	const dataObj: SimplyDocument = req.body
...
export const updateSimpleDocument = async (req: Request, res: Response, collection: string, auditProcessor: DiffProcessor | undefined = undefined) => {
	...
	const updateResult = await Mongo.getCollection(collection).updateOne(query, { $set: dataObj })
...
export const convertListToIds = async (uid: string, collection: string, listOfIds: string[]): Promise<ObjectId[]> => {
	const ids: ObjectId[] = []

	listOfIds.forEach((id: string) => {
		ids.push(parseId(id) as ObjectId)
	})

	const foundDocuments = await Mongo.getCollectionTyped<PrivacyBucket>(collection)
		.find({ uid: uid, _id: { $in: ids } })
		.toArray()

	const resultingIds: any[] = []

	foundDocuments.forEach((document) => {
		resultingIds.push(document._id)
	})
